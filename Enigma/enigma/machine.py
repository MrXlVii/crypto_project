...
class Core(object):
    ...
    def config(self, first, second, third, p1, p2, p3):
        # Sets the current rotor configuration
        # and positions of each respective Rotor.
        self.r1 = first
        self.r2 = second
        self.r3 = third
    ...
    def encrypt(self, plain):
        ...
        for i in range(len(cipher)):
            ...
            if cipher[i] is 32:
                ...
                cipher[i] -= 32  # Changes lowercase to uppercase
                temp = (cipher[i] +
                        ord(self.r1.current) +
                        ord(self.r2.current) +
                        ord(self.r3.current)) % 26
    ...
    def decrypt(self, cipher):
        ...
        for i in range(len(plain)):
            ...
            if plain[i] is 32:
                ...
                plain[i] -= 32  # Changes lowercase input to uppercase
                temp = (plain[i] -
                        ord(self.r1.current) -
                        ord(self.r2.current) -
                        ord(self.r3.current)) % 26
...
class Machine:
    ...
    def begin(self):
        ...
        q2 = Label(self.sub,
                   text='Which position will each rotor be set to?')
        q2.pack()
    ...
    def messageInput(self):
        ...
        greeting2 = Label(self.sub2,
                          text='Would you like to encrypt or' +
                               ' decrypt this message? '
                          )
        greeting2.pack()

        encryptButton = Button(self.sub2,
                               text='Encrypt', fg='black',
                               command=lambda: self.enBut(e.get())
                               )
        encryptButton.pack()
        decryptButton = Button(self.sub2,
                               text='Decrypt', fg='black',
                               command=lambda: self.deBut(e.get())
                               )
        decryptButton.pack()
    ...
    def enBut(self, plain):
        ...
        contButton = Button(self.sub3,
                            text='Continue with Same Settings',
                            command=self.same
                            )
        contButton.pack()
        diffButton = Button(self.sub3,
                            text='Continue with Different Settings',
